...
function newsbox($heading) 
{
	...
	for ($i = 0; $i < $cl; $i++) 
	{
		if ($h[$i] == $heading) 
		{
			...
			{
				$body = preg_replace('~<h1.*?class=".*?_level([1-6])_page_.*?".*?>(.*?)</h1>~isu', '', $c[$i]);
			}
			else
			{
...
function csrfProtection() 
{
	...
	{
		if($_POST['csrf_token'] !== $_SESSION[$csrfSession])die("<a href=\"./?logout\">CSRF protection - logout &raquo;</a>");
	}
}
...
function download($fl) 
{
    ...
    if (!is_readable($fl) || ($download != '' && !chkdl($sn . '?download=' . basename($fl)))) 
	{
        global $o, $text_title;
        shead('404');
		$fl = htmlspecialchars(stripslashes(strip_tags($fl)), ENT_QUOTES, 'UTF-8');
        $o .= '<p>File ' . $fl . '</p>';
        return;
    } 
	else 
...
function rfc() 
{
	...
	if($cf['use']['h1only_pagesplitting'] == 'true')
	{
		$content = preg_split('/(?=<h1.*class=".*?_level[1-6]_page_.*)/i', $content);
	}
	else
	{
...
function shead($s) 
{
    global $iis, $cgi, $tx, $cf, $title, $o, $CMSimple4menulevel;
    if ($s == '401')
...
{
    global $cf;
    return isset($cf['site']['title']) ? htmlspecialchars(strip_tags($cf['site']['title']), ENT_QUOTES, 'UTF-8') : '';
}
...
{
	global $c, $cl, $s, $l, $cf, $adm, $su, $CMSimple4menulevel;
	if (isset($start)) 
	{
		if (!isset($end))
		{
...
function locator($cmsimpleLocatorDelimiter = '&gt;') 
{
	...
	elseif ($s > 0) {
		...
		if ($tl > 1) 
		{
			for ($i = $s - 1; $i >= 0; $i--) 
			{
				if ($l[$i] < $tl) 
				{
					$t = '<span class="cmsimpleLocatorElement">' . a($i, '') . $h[$i] . '</a> ' . $cmsimpleLocatorDelimiter . ' </span> ' . "\n" . $t;
					$tl--;
				}
				if ($tl < 2)
					break;
		...
		{
			return "\n" . '<span class="cmsimpleLocatorElement">' .  a(0, '') . $tx['locator']['home'] . '</a> ' . $cmsimpleLocatorDelimiter . ' </span> ' . "\n" . $t . '<span class="cmsimpleLocatorElement cmsimpleLocatorElementLast">' . str_replace('_splitToc_','',$h[$s]) . '</span> ';
		} 
		else 
		{
			return "\n" . $t . '<span class="cmsimpleLocatorElement cmsimpleLocatorElementLast">' . str_replace('_splitToc_','',$h[$s]) . '</span>';
...
{
	global $adm, $edit, $s, $u, $sn, $tx, $sl, $cf, $su, $pth, $disabled_plugins, $active_plugins, $userfiles_path_images, $userfiles_path_downloads, $userfiles_path_media, $cmsimpleAdmin; // pluginmanager (added "$pth, $disabled_plugins")
