...
function page_params_view($page){
	...
	$view .= '<input type="hidden" name="csrf_token" value="' . $_SESSION[$csrfSession] . '">' . "\n";
...
############# alternative sitetitle #############
	$view .= "\n\t".'<a class="pl_tooltip" href="#">'.$help_icon.'<span style="padding: 10px 9px 12px 9px;">' . str_replace(tag('br').tag('br').tag('br').tag('br'),tag('br').tag('br'), str_replace("\r",tag('br'),str_replace("\n",tag('br'),$lang['hint_sitetitle']))) . '</span></a>';
	$view .= "\n\t".'<span class = "pp_label">'.$lang['sitetitle'].'</span>'.tag('br');
	...
	$view .= "\n\t\t".tag('input type="radio" name="show_sitetitle" value="1" id = "show_sitetitle_yes"'.$js. $checked). '<label for="show_sitetitle_yes">'.$lang['yes'].'</label>';
	$checked = '';
	$disabled = '';
	if($page['show_sitetitle'] !== '1'){
		$checked = $strChecked;
    ...
    $view .= "\n\t\t".tag('input type="radio" name="show_sitetitle" value="0" id = "show_sitetitle_no"'.$checked .$js). '<label for="show_sitetitle_no">'.$lang['no'].'</label>' .tag('br');
	$view .= "\n\t\t".tag('input type="text" size = "50" name="sitetitle" id = "other_sitetitle" value="'. str_replace('"', '&quot;', $page['sitetitle']).'"'.$disabled).tag('br');
	$view .= "\n\t".tag('hr');
...
################################################
	...
	$view .= "\n\t\t".tag('input type="submit" class="submit" style="float: right; margin-right: 0; cursor: pointer;" value="'.$lang['submit'].'"').tag('br');
	$view .= "\n\t".'</div>';
	$view .= "\n".'</form>';
	return $view;
}
