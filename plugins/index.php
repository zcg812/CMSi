...
MODIFIED FOR CMSimple 4.7.6 by ge-webdesign.de 2018-11
...
function PluginMenu($add='', $link='', $target='', $text='', $style=ARRAY()) 
{
	...
	if($adm)
	{
		...
		if (!isset($style['tab'])) 
		{
			$style['tab'] = 'class="cmsimple_pluginMenuTab"';
		}
		if (!isset($style['link'])) 
		{
			$style['link'] = '';
...
function PluginPrepareConfigData($var_name='', $data=ARRAY(), $plugin='') 
{
    ...
    foreach ($data as $key => $value) 
	{
        ...
        $save_data .= '[\'' . $key . '\']="' . trim(str_replace("\\'", "'", addslashes($value))) . '";' . "\n";
    }
    $save_data .= "\n?>";
    return $save_data;
}
...
{
    global $pluginloader_tx, $admin, $adm, $csrfSession, $cmsimpleAdmin, $cf, $tx;
	
	$allow_config_plugins = $cf['site']['allow_config_plugins'];
	...
	if($adm)
	{
		...
		else 
		{
			...
			$saveform .= '<input type="hidden" name="csrf_token" value="' . $_SESSION[$csrfSession] . '">' . "\n";
			...
			{
				$saveform.= '<p style="color: #900;"><b>Logged in as Admin</b></p>';
			}

			if (isset($_SESSION['cmsimpleAdmin']) && $_SESSION['cmsimpleAdmin'] == $cmsimpleAdmin || $allow_config_plugins == 'true')
			{
			...
			{
				$saveform .= '<p>' . $tx['settings']['allow_config_plugins'] . '</p>';
			}

			if ($form['type'] == 'TEXT' && isset($_SESSION['cmsimpleAdmin']) && $_SESSION['cmsimpleAdmin'] == $cmsimpleAdmin || $form['type'] == 'TEXT' && $allow_config_plugins == 'true') 
			{
				$saveform .= '<textarea ' . $style['textarea'] . ' name="' . $form['textarea_name'] . '">' . $data . '</textarea>';
			}

			if ($form['type'] == 'CONFIG' &&isset($_SESSION['cmsimpleAdmin']) &&  $_SESSION['cmsimpleAdmin'] == $cmsimpleAdmin || $form['type'] == 'CONFIG' && $allow_config_plugins == 'true') 
			{
...
function print_plugin_admin($main) 
{
    ...
    if($config == 'ON') 
	{ 
        PluginMenu('TAB', $sn.'?&amp;'.$plugin.'&amp;admin=plugin_config&amp;action=plugin_edit', '', $pluginloader_tx['menu']['tab_config'], $plugin_menu_style); 
    }

    if ($language == 'ON') 
	{
        PluginMenu('TAB', $sn . '?&amp;' . $plugin . '&amp;admin=plugin_language&amp;action=plugin_edit', '', $pluginloader_tx['menu']['tab_language'], $plugin_menu_style);
    }
    if ($help == 'ON') 
	{
        PluginMenu('TAB', $pth['file']['plugin_help'], 'target="_blank"', $pluginloader_tx['menu']['tab_help'], $plugin_menu_style);
    }
